export function initSecurityTabs() {
    ...
    const tabs = Array.from(tablist.querySelectorAll('[data-security-tab]'));
    const panels = Array.from(document.querySelectorAll('[data-security-panel]'));
    if (!tabs.length || !panels.length) return;

    const setActiveTab = (nextTab, shouldFocus = false, shouldUpdateHash = false) => {
        const target = nextTab.dataset.securityTab;
        const nextPanel = panels.find(panel => panel.dataset.securityPanel === target);
        ...
        panels.forEach(panel => {
            ...
            panel.classList.toggle('security-tabpanel--active', isActive);
        });
    ...
    if (activeFromHash) {
        requestAnimationFrame(() => tablist.closest('.security-page')?.scrollIntoView({ block: 'start' }));
    }
